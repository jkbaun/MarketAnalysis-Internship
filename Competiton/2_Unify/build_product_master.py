...
# =====================================================================
# EXTRACTION, BRAND RESOLUTION & STATE HYDRATION
# =====================================================================
def extract_staging_variants(staging_db_dir: Path):
    ...
    for db_file in staging_db_dir.glob("competitor_*.db"):
        ...
        query = """
            SELECT 
                p.id as product_id, p.title as product_title, p.vendor,
                GROUP_CONCAT(t.name) as raw_tags, v.id as variant_id,
                v.title as variant_title, v.sku, v.barcode, v.price, v.compare_at_price
            FROM variants v
    ...
    if bbk_db.exists():
        ...
        query = """
            SELECT 
                p.upk_id as product_id, p.product_title, p.vendor, '' as raw_tags,
                v.variant_id, v.variant_title, v.sku, v.variant_barcode as barcode,
                0.0 as price, 0.0 as compare_at_price
...
def load_existing_state(session):
    """
    Hydrates memory dictionaries from the Database to ensure idempotency.
    Returns tracking dictionaries and the correctly calibrated max UPK counter.
    """
    upk_by_barcode = {}
    ...
    # 1. Load Existing UPKs (DimUnifiedProduct)
    for upk in session.query(DimUnifiedProduct).all():
        b_id = upk.brand_id or "BRD-UNK-0000"
        spec = upk.extracted_spec or ""
        core_key = upk.clean_match_key or ""
        
        if upk.canonical_barcode:
            upk_by_barcode[upk.canonical_barcode] = upk.upk_id
            
        if core_key:
            upk_by_core_spec_brand[(core_key, spec, b_id)] = upk.upk_id
            
            key = (b_id, spec)
            if key not in upk_fuzzy_registry:
                upk_fuzzy_registry[key] = []
            upk_fuzzy_registry[key].append((core_key, upk.upk_id))
            
        # Extrapolate max counter from UPK formats (e.g., UPK-COS-1052-50ML)
        parts = upk.upk_id.split('-')
        if len(parts) >= 3:
            try:
                c = int(parts[2])
                if c > max_counter:
                    max_counter = c
                if core_key and core_key not in core_to_counter_map:
                    core_to_counter_map[core_key] = c
            except ValueError:
                pass
                
    upk_counter = max_counter + 1
    
    # 2. Load Existing Store Variants (FactStoreVariant) to avoid re-insertion
    link_id_map = {}
    existing_variant_keys = set()
    
    for v in session.query(FactStoreVariant).all():
        c_var_id = clean_id_str(v.store_variant_id)
        c_prod_id = clean_id_str(v.store_product_id)
        
        existing_variant_keys.add((v.origin_company, c_var_id))
        
        link_id_map[(v.origin_company, c_var_id)] = (v.link_id, v.upk_id, v.currency_raw or "BHD")
        link_id_map[(v.origin_company, c_prod_id)] = (v.link_id, v.upk_id, v.currency_raw or "BHD")
        
    return upk_by_barcode, upk_by_core_spec_brand, upk_fuzzy_registry, core_to_counter_map, upk_counter, link_id_map, existing_variant_keys
...
def main():
    ...
    print("[+] Loading existing State for Idempotency...")
    (upk_by_barcode, upk_by_core_spec_brand, upk_fuzzy_registry, 
     core_to_counter_map, upk_counter, link_id_map, existing_variant_keys) = load_existing_state(session)
    ...
    for _, row in variants_df.iterrows():
        ...
        var_id_clean = clean_id_str(row['variant_id'])
        
        # --- Idempotency Gate ---
        # If the variant is already recorded, skip generation and lineage mapping
        if (company, var_id_clean) in existing_variant_keys:
            continue
            
        existing_variant_keys.add((company, var_id_clean)) # Add immediately to prevent intra-run duplicates
        ...
        # Update Lookup Registries so new variants in this session can reference newly created UPKs
    ...
    session.commit()
    print(f"[+] Created {new_upks_created} New Core Products and Matched {len(store_variant_records)} New Store Variants.")
    ...
    for rec in store_variant_records:
        ...
        # Add newly created links into mapping for the Snapshot Phase
    ...
    # Existing Snapshot Guard to Avoid Duplicate Writes
    ...
    for db_file in staging_db_dir.glob("competitor_*.db"):
        ...
        query = """
                SELECT 
                    ...
                    s.snapshot_date, 
                    s.stock_quantity, 
                    COALESCE(v.price, 0.0) as price 
                FROM stock_snapshots s
                LEFT JOIN variants v ON CAST(s.variant_id AS TEXT) = CAST(v.id AS TEXT)
                """
    ...
    session.commit()
    print(f"[SUCCESS] Product Master Sync Completed!")
    print(f"          Total New UPKs Initialized: {new_upks_created}")
    print(f"          Total New Store Variants Added: {len(store_variant_records)}")
